                               				...
                               				android_thread_id_t *threadId)
{
    ...
    if (result != 0) {
        ALOGE("androidCreateRawThreadEtc failed (entry=%p, res=%d, errno=%d)\n"
             "(android threadPriority=%d)", entryFunction, result, errno, threadPriority);
        return 0;
    }
...
int androidCreateThreadEtc(android_thread_func_t entryFunction,
                            ...
                            android_thread_id_t *threadId)
{
    return gCreateThreadFn(entryFunction, 
							userData, 
							threadName,
        					threadPriority, 
        					threadStackSize, 
...
status_t Thread::run(const char* name, int32_t priority, size_t stack)
{
    ...
    if (mRunning) {			/* 判断该线程是否已经运行起来 */
        return INVALID_OPERATION;		/* 如果已经起来,直接返回 */
    }
	...
	/* 创建线程: 给定了线程的执行函数_threadLoop
	 * createThreadEtc
	 * 	-> androidCreateThreadEtc
	 *	    -> gCreateThreadFn (初始化值:androidCreateRawThreadEtc, Zygote启动时: javaCreateThreadEtc)
	 *		本质: androidCreateRawThreadEtc	
	 * 其中多了一步:javaAttachThread, 这样该线程能执行Java代码
	 *
