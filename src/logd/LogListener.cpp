...
bool LogListener::onDataAvailable(SocketClient *cli) {
	...
	printf("%s, %d recv size is too short.\n", __func__, __LINE__);
        return false;
    }
    ...
    if (cred == NULL) {
	printf("%s, %d cred is null, what's wrong\n", __func__, __LINE__);

        return false;
    }

    if (cred->pid == getpid() /*cred->uid == getuid()*/) {	/* Android上没有进程的用户ID是不一致的,但是在Ubuntu上不一样的 */


        // ignore log messages we send to ourself.
        // Such log messages are often generated by libraries we depend on
        // which use standard Android logging.
	printf("%s, %d cred->uid equal getuid()\n", __func__, __LINE__);
