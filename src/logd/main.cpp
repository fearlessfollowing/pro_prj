...
int main() 
{
	...
	printf("LogReader startListener failed, exit now\n");
        exit(1);
    }
    ...
    if (cl->startListener()) {
        printf("CommandListener startListener failed, exit now.\n");
        exit(1);
    }
	...
	property_set("sys.logd.startup", "true");	/* 通知系统logd复位已经启动完成 */
