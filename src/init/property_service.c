...
static int init_workspace(workspace *w, size_t size)
{
    ...
    int fd;
         ...
         */
    fd = open(PROPERTY_SPACE_PATH, O_RDWR | O_CREAT, 0600);
    if (fd < 0)
        return -1;
    ...
    fd = open(PROPERTY_SPACE_PATH, O_RDONLY);
    if (fd < 0)
        return -1;

	/* 为了让其他进程能够访问属性文件,此处不删除该文件,将其改为隐藏文件 */
    //unlink(PROPERTY_SPACE_PATH);
...
static int check_perms(const char *name, unsigned int uid, unsigned int gid, char *sctx)
{
    ...
    for (i = 0; property_perms[i].prefix; i++) {
        if (strncmp(property_perms[i].prefix, name, strlen(property_perms[i].prefix)) == 0) {
            if ((uid && property_perms[i].uid == uid) || (gid && property_perms[i].gid == gid)) {
                return check_mac_perms(name, sctx);
            }
        }
    }
...
const char* _property_get(const char *name)
{
    prop_info *pi;

    if (strlen(name) >= PROP_NAME_MAX) 
        return 0;
...
/*
 * 服务端设置属性(monitor进程内部使用)
 * 其他进程如果需要设置属性,需要通过套接字连接到monitor进程,但是读属性不需要
 */
int _property_set(const char *name, const char *value)
{
    prop_area *pa;
    prop_info *pi;
        ...
        case PROP_MSG_SETPROP: {
            ...
            if (memcmp(msg.name,"ctl.",4) == 0) {
                ...
                if (check_perms(msg.name, cr.uid, cr.gid, source_ctx)) {
                    _property_set((char*) msg.name, (char*) msg.value);
                } else {
    ...
    if (dir) {
        while ((entry = readdir(dir)) != NULL) {
            ...
            if (fd >= 0) {
                ...
                if (length >= 0) {
                    value[length] = 0;
                    _property_set(entry->d_name, value);
                } else {
